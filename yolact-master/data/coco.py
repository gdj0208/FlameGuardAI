...
class COCODetection(data.Dataset):
    ...
    def pull_item(self, index):
        ...
        img = cv2.imread(path)
        print(f"Debug: Reading image from path: {path}") # 읽으려는 경로 출력
        print(f"Debug: cv2.imread result type: {type(img)}, shape: {img.shape if img is not None else None}") # 읽은 결과 타입/형태 출력
